...
/*
  cbar, fork of forked lemonbar with all the stdout-parsing functionality stripped
  
  - forked from https://github.com/krypt-n/bar, maintainer: krypt-n@github
  --> forked from: https://github.com/LemonBoy/bar, maintainer: LemonBoy@github
*/
...
RENDER_FUNC;
FONT_SET_TYPE(cfg_fonts) = CFG_FONTS;
...
rgba_t bar_mkcolor(const char* str) {
    return parse_color(str, NULL, (rgba_t)0x00000000U);
}

void
bar_attribute (uint32_t flags)
{
    attrs = flags;
}
...
static struct lncolor_elem ln[sizeof(uint8_t)];

static void setsegment(char c) {
    switch (ln[(uint8_t) c].flag) {
    case BAR_BG:
        bar_bgcolor(ln[(uint8_t) c].c);
        break;
    case BAR_FG:
        bar_fgcolor(ln[(uint8_t)c].c);
        break;
    case BAR_U:
        bar_ucolor(ln[(uint8_t) c].c);
        break;
    }
}
...
void bar_colorsegment(const char* str) {
    ...
    while((*(++c) != '\0')) {
        if (*c == '#') {
            if (c != str && *(c - 1) == '\\') { /* escaped */
                if (c - 2 >= str)
                    bar_putsn(start, (c - 1) - start);
                start = c + 1;
            } else {
                if (c + 2 <= end) { /* characters after #n */
                    if (c - 1 >= str)
                        bar_putsn(start, c - start);
                    start = c + 2;
                    setsegment(*(c + 1));
                } else if (c == end) { /* # is the last character */
                    bar_puts(start);
                    return;
                } else {               /* #n is the last two characters */
                    if (c - 1 >= str)
                        bar_putsn(start, c - start);
                    setsegment(*(c + 1));
                    return;
...
void bar_lncolor(char c, enum bar_cflag flag, rgba_t color) {
    ln[(uint8_t) c].flag = flag;
    ln[(uint8_t) c].c = color;
}

void bar_printf(const char* str, ...) {
    va_list argptr;
    va_start(argptr, str);
    char buf[4096];
    vsnprintf(buf, sizeof(buf) / sizeof(char), str, argptr);
    va_end(argptr);
    bar_colorsegment(buf);
}

void bar_vprintf(const char* str, va_list argptr) {
    char buf[4096];
    vsnprintf(buf, sizeof(buf) / sizeof(char), str, argptr);
    bar_colorsegment(buf);
}
...
void bar_leftalign(void) { pos_x = 0; align = ALIGN_L; }
void bar_rightalign(void) { pos_x = 0; align = ALIGN_R; }
void bar_centeralign(void) { pos_x = 0; align = ALIGN_C; }

void bar_bgcolor(rgba_t c) { bgc = c; update_gc(); }
void bar_fgcolor(rgba_t c) { fgc = c; update_gc(); }
void bar_ucolor(rgba_t c)  { ugc = c; update_gc(); }
...
{
    draw_shift(cur_mon, pos_x, align, offset);

    pos_x += offset;
    area_shift(cur_mon->window, align, offset);
}
...
void bar_puts(const char* c) { bar_putsn(c, strlen(c)); }

void
bar_putsn(const char* str, size_t amt)
{
    const char* p;
    for (p = str; p < str + amt;) {
...
{
    struct pollfd pollin[1] = { { .fd = -1, .events = POLLIN } };
    ...
    wm_name = CFG_WM_NAME;
    if (CFG_GEOMETRY)
        parse_geometry_string(CFG_GEOMETRY, geom_v);
    if (CFG_IS_ON_BOTTOM)
        topbar = false;
    if (CFG_FORCE_DOCKING)
        dock = true;
    size_t t;
    
    for (t = 0; t < FONT_SET_LENGTH(cfg_fonts); ++t) {
        font_load(FONT_SET_GET(cfg_fonts, t));
    }

    bu = CFG_UNDERLINE_HEIGHT;
    if (CFG_VOFFSET)
        add_y_offset(CFG_VOFFSET);
    ...
    for (;;) {
        ...
        int p;
        if ((p = poll(pollin, sizeof(pollin) / sizeof(struct pollfd), timeout)) > 0) {
            /*
              parse(input);
              redraw = true;
             */
            ...
            /* poll timeout, render again */
            timeout = render_state();
            /* ignore redrawing for valid timeouts */
            if (timeout > 0) {
                redraw = true;
            }
        } else if (errno != EINTR) {
